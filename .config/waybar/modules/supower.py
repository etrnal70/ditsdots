...
PROPERTIES = {
    'BatteryLevel':
    ('unknown', 'none', 'low', 'critical', 'normal', 'high', 'full'),
    'Capacity':
    None,
    'Energy':
    None,
    ...
    None,
    'Type':
    ('unknown', 'line-power', 'battery', 'ups', 'monitor', 'mouse', 'keyboard',
     'pda', 'phone', 'media-player', 'tablet', 'computer', 'gaming_input',
     'pen', 'touchpad', 'modem', 'network', 'headset', 'speakers',
     'headphones', 'video', 'other_audio', 'remote_control', 'printer',
     'scanner', 'camera', 'wearable', 'toy', 'bluetooth-generic'),
    'UpdateTime':
    None,
    'Vendor':
    None,
    ...
    None,
    'WarningLevel':
    ('unknown', 'none', 'discharging', 'low', 'critical', 'action')
}
...
def device_info(bus, device):
    ...
    for _property, friendly_name in PROPERTIES.items():
        try:
            data = device_interface.Get('org.freedesktop.UPower.Device',
                                        _property)
            if _property == 'UpdateTime':
                result[_property] = time.ctime(data)
            else:
                result[
                    _property] = friendly_name[data] if friendly_name else data

            if _property == 'TimeToEmpty' or _property == 'TimeToFull':
                if data >= 3600:
                    result[_property] = time.strftime('%-I hour %M minutes',
                                                      time.gmtime(data))
                elif data > 0:
                    result[_property] = time.strftime('%M minutes',
                                                      time.gmtime(data))
                else:
                    result[_property] = "Fully Charged"
...
def get_devices(bus):
    ...
    devices_proxy = bus.get_object('org.freedesktop.UPower',
...
def main(list_devices, check, device, text, alt, tooltip, _class, percentage):
    ...
    try:
        ...
        output = {
            ...
            alt.format(**info),
            "tooltip":
            (tooltip if tooltip else get_tooltip(info['Type'])).format(**info),
            "class":
            _class.format(**info),
            "percentage":
            float(percentage.format(**info))
        }
    except Exception as error:
        output = {
            "text": f'Error {device.split("/")[-1]} {error}',
            'tooltip': f'{error}'
        }
        exit = 2
